...
<head>
    ...
    <script type="text/javascript">
        function changePageSize() {
            ...
            //向服务器发送请求，改变每页显示条数
            location.href = "${pageContext.request.contextPath}/manager/managerWork?page=1&size="
                + pageSize;
        }
...
<%@include file="managerNavbar.jsp"%>
    <div style="margin:30px 100px 20px 100px;min-height: 500px">
        <table class="layui-table">
            ...
            <tbody>
            <c:forEach items="${pageInfo.list}" var="work" varStatus="loop">
                <tr>
                    <td>${loop.index+1}</td>
                    <td>${work.menuName}</td>
                    <td><fmt:formatDate value="${work.workTime}" pattern="yyyy-MM-dd"/></td>
                    <td>${work.summary}</td>
                    <td><button type="button" class="layui-btn layui-btn-radius layui-btn-sm" onclick="showWorkInfo(${work.workId})">查看</button>
                        <button type="button" class="layui-btn layui-btn-radius layui-btn-sm layui-btn-danger" onclick="deleteWork(${work.workId})">删除</button>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        ...
        <div style="text-align: center;margin-top: 20px">
            <%--首页--%>
            <a href="${pageContext.request.contextPath}/manager/managerWork?page=1&size=${pageInfo.pageSize}">
                <button class="layui-btn layui-btn-primary layui-btn-sm">首页</button>
            </a>
            <%--上一页--%>
            <c:if test="${pageInfo.hasPreviousPage}">
                <a href="${pageContext.request.contextPath}/manager/managerWork?page=${pageInfo.pageNum-1}&size=${pageInfo.pageSize}">
            ...
            <c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
                <c:if test="${pageNum == pageInfo.pageNum}">
                    <a href="${pageContext.request.contextPath}/manager/managerWork?page=${pageNum}&size=${pageInfo.pageSize}">
            ...
            <%--尾页--%>
            <a href="${pageContext.request.contextPath}/manager/managerWork?page=${pageInfo.pages}&size=${pageInfo.pageSize}">
